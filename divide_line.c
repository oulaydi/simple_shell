...
/**
 * swap_a_char - swaps | and & for non-printed chars
 *
 * @input: input string
 * @bool: type of swap
...
/**
 * append_nodes - add separators and command lines in the lists
 *
...
void append_nodes(sepa_list **head_s, line_list **head_l, char *input)
{
	...
	input = swap_a_char(input, 0);

	for (i = 0; input[i]; i++)
	{
		if (input[i] == ';')
			append_sepa_node_end(head_s, input[i]);
	...
	line = _str_tok(input, ";|&");
	do {
		line = swap_a_char(line, 1);
		append_line_node_end(head_l, line);
		line = _str_tok(NULL, ";|&");
	} while (line != NULL);
...
/**
 * divide_cmds - splits command lines according to
...
int divide_cmds(all_data_shell *datash, char *input)
{
	...
	append_nodes(&head_s, &head_l, input);
	...
	while (list_l != NULL)
	{
		...
		datash->args = divide_line(datash->input);
...
/**
 * divide_line - tokenizes the input string
 *
 * @input: input string.
